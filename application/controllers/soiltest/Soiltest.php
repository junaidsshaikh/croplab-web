...
class Soiltest extends MY_Controller {
	...
	public function get_reports_by_mobile_num(){
			...
			//print_r($farmer_info->farmer_id);
				 //For Plant
				 if($this->uri->segment(4)=='plant'){
					if($farmer_info){
						$this->load->view("downloads/plant_inward_info_view_by_farmer",["farmer_info"=>$farmer_info, "sampleid"=>$sampleid]);
					}else{
				 ...
				 //For Soil
				 if($this->uri->segment(4)=='soil'){
					if($farmer_info){
						$this->load->view("downloads/soil_inward_info_view_by_farmer",["farmer_info"=>$farmer_info]);
					}else{
					echo "Mobile Number Not Exist";
				 ...
				 //For Water
				 if($this->uri->segment(4)=='water'){
					if($farmer_info){
						$this->load->view("downloads/water_inward_info_view_by_farmer",["farmer_info"=>$farmer_info]);
					}else{
					echo "Mobile Number Not Exist";
	...
	public function get_reports_by_farmer($id, $sampleid) {
		...
		foreach($plantinwards as $inward) {
				$action = '
			<a href="'.base_url("soiltest/soiltest/get_plant_analysis_info/".$inward["plant_inward_sample_id"]) .'" class="btn btn-primary btn-sm" title="Print"><i class="dripicons-download">	</i>Download</a>	
			';
	...
	public function get_plant_analysis_info($plantSampleId) {
		if($this->if_plant_sample_id_exist($plantSampleId)){
    		...
    		if($stage_details) {
    			$this->load->view("downloads/preview_plant_analysis_info_view", ["stage_details"=> $stage_details, "plant_calculation"=> $plant_calculation, "farmer"=>$farmer_inward]);
	...
	public function get_soil_inward_info($id) {
		...
		foreach($soilinwards as $inward) {
				$action = '<a href="'.base_url("soiltest/soiltest/get_soil_analysis_info/".$inward["soil_inward_sample_id"]) .'" class="btn btn-primary btn-sm" title="Print"><i class="dripicons-download">		</i>Download</a>	';
	...
	public function get_soil_analysis_info($soilSampleId) {
		...
		$this->load->view("downloads/preview_soil_analysis_info_view", ["stage_details"=> $stage_details, "soil_calculation"=> $soil_calculation, "farmer"=>$soil_inward]);
				$html = $this->output->get_output();
	...
	public function get_water_inward_info($id) {
		...
		foreach($waterinwards as $inward) {
				$action = '<a href="'.base_url("soiltest/soiltest/get_water_analysis_info/".$inward["water_inward_sample_id"]) .'" class="btn btn-primary btn-sm" title="Print"><i class="dripicons-download">		</i>Download</a>';
	...
	public function get_water_analysis_info($waterSampleId) {
		...
		$this->load->view("downloads/preview_water_analysis_info_view", ["stage_details"=> $stage_details, "water_calculation"=> $water_calculation, "farmer"=>$water_inward]);
				$html = $this->output->get_output();
